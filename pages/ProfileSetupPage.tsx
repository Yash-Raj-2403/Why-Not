    ...
    <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-4">
      <div className="max-w-2xl w-full bg-slate-900 p-8 rounded-xl border border-slate-800">
        ...
        <div className="flex justify-between mb-8">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className={`w-8 h-8 rounded-full flex items-center justify-center ${step >= i ? 'bg-indigo-600' : 'bg-slate-700'}`}>
              {i}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <div className="mt-6 flex justify-end">
              <button onClick={nextStep} className="bg-indigo-600 px-4 py-2 rounded hover:bg-indigo-700">Next</button>
            </div>
          </motion.div>
        )}
        ...
        {step === 4 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <div className="mt-6 flex justify-between">
              <button onClick={prevStep} className="text-slate-400 hover:text-white">Back</button>
              <button onClick={handleSubmit} className="bg-indigo-600 px-6 py-2 rounded hover:bg-indigo-700 font-bold">Complete Profile</button>
            </div>
          </motion.div>
        )}
      </div>
